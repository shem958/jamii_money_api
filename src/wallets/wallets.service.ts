...
export class WalletsService {
    ...
    async findOne(id: string) {
        const wallet = await this.walletModel.findById(id).populate('userId', 'name email phone');
        if (!wallet) throw new NotFoundException('Wallet not found');
        return wallet;
    }

    async update(id: string, updateWalletDto: any) {
        const updated = await this.walletModel.findByIdAndUpdate(id, updateWalletDto, { new: true });
