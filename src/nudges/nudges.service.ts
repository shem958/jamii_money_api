...
export class NudgesService {
    constructor(@InjectModel(Nudge.name) private readonly nudgeModel: Model<Nudge>) { }
    ...
    async findOne(id: string) {
        const nudge = await this.nudgeModel.findById(id).populate('userId', 'name email');
        if (!nudge) throw new NotFoundException('Nudge not found');
        return nudge;
    }

    async update(id: string, updateNudgeDto: any) {
        const updated = await this.nudgeModel.findByIdAndUpdate(id, updateNudgeDto, { new: true });
