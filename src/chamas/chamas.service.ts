...
export class ChamasService {
    constructor(@InjectModel(Chama.name) private readonly chamaModel: Model<Chama>) { }
    ...
    async findOne(id: string) {
        const chama = await this.chamaModel.findById(id).populate('members', 'userId role amount');
        if (!chama) throw new NotFoundException('Chama not found');
        return chama;
    }

    async update(id: string, updateChamaDto: any) {
        const updated = await this.chamaModel.findByIdAndUpdate(id, updateChamaDto, { new: true });
